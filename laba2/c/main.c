...
#define MASTER_BASE_VECTOR 0x70 
...
void init_new_handlers(void) {
    ...
    outp(0x20, 0x11);        // ICW1 
    outp(0x21, 0x70);        // ICW2 (изменено на 0x70)
    outp(0x21, 0x04);        // ICW3
    outp(0x21, 0x01);        // ICW4
...
int main(void) {
    ...
    _dos_freemem(*fp);
    _dos_keep(0, (_DS - _CS) + (_SP / 16) + 1); // первый параметр - условие выхода из резидентной программы, второй параметр - кол-во выделенной памяти
