...
PREPARE_RTC:                                    ;Подготовка часов RTC                                        ;1 - корректное чтение времени в двоичном формате 
    ; Установить двоичный формат и 24-часовой режим
    mov al,0Bh
    out 70h,al
    in  al,71h
    or  al,00000100b        ; Установить бит DM (двоичный формат)
    and al,11110111b        ; Сбросить бит 12/24 (24-часовой режим)
    out 71h,al
    
    ; Разрешить прерывания от RTC
    mov al,0Ah
    out 70h,al
    in al,71h
    and al,11110000b        ; Сохранить верхние 4 бита (rate selection)
    or al,00000110b         ; Установить биты 1 и 2 (разрешить периодические прерывания)
    out 71h,al
    
    ; Установить частоту прерываний (например, 2 Hz)
    mov al,0Ah
    out 70h,al
    in al,71h
    and al,11110000b        ; Сохранить верхние 4 бита
    or al,00001110b         ; Установить rate = 0110 (2 Hz)
...
FILL_IDT:                                       ;Заполнить таблицу дескрипторов шлюзов прерываний
    ...
    endm
   lea eax, TIMER_HANDLER                      ;Поместить обработчик прерывания RTC на 20 шлюз
...
REINITIALIAZE_CONTROLLER_FOR_PM:                ;Переинициализировать контроллер прерываний на вектора 20h, 28h
    ...
    ; Размаскировать прерывание RTC (IRQ8)
    in al,0A1h
    and al,11111110b        ; Размаскировать IRQ8 (бит 0)
    out 0A1h,al
    out 21h,al                                  ;Ведущего контроллера
    out 0A1h,al                                 ;Ведомого контроллера
...
TIMER_HANDLER proc near                         ;Обработчик прерываний RTC
    push ds
    push es
    ...
    ; Считать регистр C чтобы очистить прерывание
    mov al,0Ch
    out 70h,al
    in al,71h
    ...
    cmp al,ds:[SECOND]                          ;Если секунда изменилась
    je SKIP_SECOND                              ;Если нет - пропустить
    
    mov ds:[SECOND],al                          ;Сохранить новое значение секунд
    
    cmp ds:[TIME],0                             ;Если время вышло
    je DISABLE_PM                               ;Выйти из PM
    
    dec ds:[TIME]                               ;Уменьшить оставшееся время
    
    ; Вывести оставшееся время
    lea edi,BUFFER_TIME
    mov al,ds:[TIME]
    xor ah,ah
    call WORD_TO_DEC
...
DISABLE_PM:                                     ;Выход из защищенного режима
    mov al,20h
    out 0A0h,al                                ;Послать EOI ведомому контроллеру
    out 20h,al                                 ;Послать EOI ведущему контроллеру
    db 0eah                                    ;Дальний jmp
